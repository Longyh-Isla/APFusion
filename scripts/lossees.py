...
class fusion_loss(nn.Module):
    ...
    def forward(self, image_visible, image_infrared, image_fused, max_ratio=4, ssim_vis_ratio=1, ssim_ir_ratio=1,
                ssim_ratio=1, color_ratio=12, text_ratio=10):
        ...
        loss_ssim = ssim_ratio * (ssim_vis_ratio * self.loss_func_ssim(image_visible,
                                                                       image_fused) + ssim_ir_ratio * self.loss_func_ssim(
        ...
        total_loss = loss_max + loss_ssim + loss_text
        return total_loss, loss_ssim, loss_max, loss_text
...
class fusion_prompt_loss(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused, task):
        ...
        for idx, task_type in enumerate(task):
            ...
            if task_type == "vi_ir":
                loss, ssim_loss, max_loss, grad_loss = self.fusion_loss(img_A, img_B, img_fused,
        ...
        return total_loss / num_tasks, total_ssim_loss / num_tasks, total_max_loss / num_tasks, total_grad_loss / num_tasks,

    def _get_image(self, images, index):
...
class L_Intensity_Consist(nn.Module):
    ...
    def forward(self, image_visible, image_infrared, image_fused, vis_compose, ir_compose, consist_mode="l1"):
        if consist_mode == "l2":
            Loss_intensity = (vis_compose * F.mse_loss(image_visible, image_fused) + ir_compose * F.mse_loss(
                image_infrared, image_fused)) / 2
        else:
            Loss_intensity = (vis_compose * F.l1_loss(image_visible, image_fused) + ir_compose * F.l1_loss(
                image_infrared, image_fused)) / 2
        return Loss_intensity
