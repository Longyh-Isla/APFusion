...
from model.APFusion_model import APFusion as create_model
...
def main(args):
    ...
    file_name = "Concat_resume"
    # file_name = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    filefold_path = "./experiments/APFusion_train_{}".format(file_name)
    ...
    print("Loading IVF Fusion and vi_ir Task!")
    if args.vi_ir_path is not None:
        train_vi_ir_path_list, val_vi_ir_path_list = read_data(args.vi_ir_path)
    else:
        train_vi_ir_path_list = val_vi_ir_path_list = None
    ...
    train_dataset = PromptDataSet(train_vi_ir_path_list=train_vi_ir_path_list,
                                  val_vi_ir_path_list=val_vi_ir_path_list,
                                  phase="train",
                                  transform=data_transform["train"])

    val_dataset = PromptDataSet(train_vi_ir_path_list=train_vi_ir_path_list,
                                val_vi_ir_path_list=val_vi_ir_path_list,
                                phase="val",
    ...
    for epoch in range(start_epoch, args.epochs):
        # train
        train_loss, train_ssim_loss, train_int_loss, train_text_loss, lr = train_one_epoch(model=model,
        ...
        tb_writer.add_scalar("train_int_loss", train_int_loss, epoch)
        ...
        if epoch % args.val_every_epcho == 0 and epoch != 0:
            val_loss, val_ssim_loss, val_int_loss, val_text_loss = evaluate(model=model,
            ...
            tb_writer.add_scalar("val_int_loss", val_int_loss, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vi_ir_path', type=str, default="datasets/train/vi_ir")

    parser.add_argument('--weights', type=str, default='',
                        help='initial weights path')
    parser.add_argument('--val_every_epcho', type=int, default=2, help='val every epcho')
    parser.add_argument('--resume', default='experiments/APFusion_train_Concat/weights/checkpoint_lastest.pth', help='')
